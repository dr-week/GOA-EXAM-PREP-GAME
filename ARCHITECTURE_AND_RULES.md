...
3. **Open-Source Design System**: Use `clsx` and `tailwind-merge` via `src/utils/cn.ts` to dynamically combine Tailwind classes without utility conflicts. Use `canvas-confetti` for achievement celebration particles.
4. **Tailwind First**: Do not write custom CSS unless absolutely necessary for complex animations. Use Tailwind utility classes.
5. **Sub-Second Test Verification**: Any code modification must be verified by running `python src/manage_db.py ci` (full single-command pipeline) or `cd frontend && npm test` (sub-second Vitest suite).
...
|---|---|---|---|
| **Run Full Parallel CI Chain** | `python src/manage_db.py ci` | **~3.2s** | Runs Audit ➔ Compile ➔ Pytest ➔ Vitest ➔ React Build. |
| **Run Sub-Second Vitest Suite** | `cd frontend && npm test` | **~246ms** | Runs React component unit tests (state, audio, i18n). |
| **Run Pytest Suite** | `python src/manage_db.py test` | **< 2.0s** | Runs all 11 backend, database, & frontend schema tests. |
